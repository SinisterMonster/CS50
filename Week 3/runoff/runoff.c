...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            //  If candidate not elim then update vote and break out of j loop
            // Else j will make it go into next pref of voter
...
bool print_winner(void)
{
    ...
    // Find majority required
    int majority = voter_count / 2;

    // Find index of max votes and print winner
